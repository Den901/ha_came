...
class ScenarioManager:
    ...
    async def get_scenarios(self):
        ...
        response = await self._manager.application_request(
            {"cmd_name": "scenarios_list_req"}, "scenarios_list_resp"
        )
        scenarios = response.get("array", [])

        _LOGGER.debug("ScenarioDevice._get_scenarios restituisce %d scenari", len(scenarios))
        return scenarios
    

    async def activate_scenario(self, scenario_id: int):
        """Attiva uno scenario esistente."""
        try:
            await self._manager.application_request(
    ...
    def handle_update(self, hass, device_info: dict):
        ...
        if cmd_name == "scenario_status_ind":
            ...
            hass.add_job(self.refresh_scenarios())
            hass.add_job(
                async_dispatcher_send,
                hass,
